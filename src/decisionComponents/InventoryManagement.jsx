import {
  ...
  function InventoryManagement() {
    ...
    const mapInputsToObject = () => {
      ...
      data.forEach((row, rowIndex) => {
        ...
        row.slice(2).forEach((cell, colIndex) => {
          ...
          resultObject[supplierKey] = inputValues[colIndex][rowIndex];
        });
      });
    ...
    //     try {
    //       const response = await fetch("http://127.0.0.1:8000/api/product_data/");
    ...
    return (
      <>
        <Box>
          <Box>
          <Text display={"inline"} fontSize={'1.5rem'}>Inventory Management Decision form | </Text>
